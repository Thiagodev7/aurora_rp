...
vRP._prepare("attachs/DELETE","DELETE FROM skips_attachs WHERE user_id = @user_id")
	...
	if vRP.hasPermission(user_id,skips['permgodadmin']) then
        if args[1] then
			...
			if nplayer then
				...
				vRP.setThirst(nplayer,-100)
				vRP.setHunger(nplayer,-100)
			...
			TriggerClientEvent("resetBleeding",source)
               ...
               vRPclient.killGod(source)
				...
				vRP.setThirst(source,-100)
				vRP.setHunger(source,-100)
