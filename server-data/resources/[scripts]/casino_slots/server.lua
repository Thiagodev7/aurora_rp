...
AddEventHandler('casino:taskStartSlots',function(index, data)
	...
	local user_id = vRP.getUserId(source)
	if user_id ~= nil then
	if vRP.getInventoryItemAmount(user_id,"ficha") < data.bet then
		...
		if activeSlot[index] then
			vRP.tryGetInventoryItem(user_id,"ficha",data.bet,false)
...
function getPlayerChips(source)
	...
	local user_id = vRP.getUserId(source)
    if user_id then
		print(vRP.getInventoryItemAmount(user_id,"ficha"))
