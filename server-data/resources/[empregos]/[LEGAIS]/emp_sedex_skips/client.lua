...
job = Tunnel.getInterface("emp_sedex_skips")
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIAVEIS
-----------------------------------------------------------------------------------------------------------------------------------------
...
-- INICIANDO TRABALHO
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent('emp_sedex_skips:permissao')
AddEventHandler('emp_sedex_skips:permissao',function()
...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(GetEntityCoords(ped), -424.05,-2789.53,6.39,true) <= 30 then
			sleep = 1
		   DrawText3D(-424.05,-2789.53,6.39+0.47, "~w~Sedex", 1.0, 4)
		  --  DrawText3D(-424.05,-2789.53,6.39+0.33, "~b~Post Up", 0.9, 1)
		    DrawMarker(21, -424.05,-2789.53,6.39-0.1, 0, 0, 0, 0, 0, 0, 0.5, 0.5, 0.5, 255, 80, 0, 90, 0, true, false, false)
		    if GetDistanceBetweenCoords(GetEntityCoords(ped), -424.05,-2789.53,6.39,true) <= 1 then
                if IsControlJustPressed(0,38) then	
				    TriggerServerEvent('emp_sedex_skips:permissao')
					Fade(1200)
		...
		-- sleep = 1
		--         DrawMarker(21,-411.87,-2797.83,6.00-0.6,0,0,0,0,180.0,130.0,2.0,2.0,1.0,255,80,0,90,1,0,0,1)
		...
		if emservico and parte == 1 then
			...
			if distance <= 50 and not pegando and portaaberta then
				DrawMarker(21,-439.62,-2796.29,7.29-0,0,0,0,0,180.0,130.0,1.0,1.0,1.0,255,80,0,90,1,0,0,1)
	...
	while true do
		...
		if emservico and parte == 2 then
			...
			if distance2 <= 50 and pegando and encomendapega then
				if distance2 < 10.0 and not IsPedInAnyVehicle(ped) then
					sleep = 5
					DrawMarker(21,entregalocal[destino].x,entregalocal[destino].y,entregalocal[destino].z-0.6,0,0,0,0,180.0,130.0,2.0,2.0,1.0,255,80,0,90,1,0,0,1)
				--	DrawText3Ds(entregalocal[destino].x,entregalocal[destino].y,entregalocal[destino].z+0.35,"~b~[E] ~w~Entregar Encomenda")
                    ...
                    if IsControlJustPressed(0,38) then
                    	if quantidade >= 0 then
                            ...
                            vRP._DeletarObjeto()
                            ClearPedTasksImmediately(ped)
                            TriggerServerEvent("emp_sedex_skips:receber",pagamento)
                            portaaberta = false
                            RemoveBlip(blip)
                        	if quantidade > 0 then
...
function FadeRoupa(time,tipo,idle_copy)
	...
	if tipo == 1 then 
		...
		TriggerServerEvent("emp_sedex_skips:roupa")
	end
	DoScreenFadeIn(800)
end
